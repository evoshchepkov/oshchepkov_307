...
def reflections_try():
    ...
    cyan_ball = Sphere(Point(0.75, -0.1, 1.0), 0.6, Material(Color.from_hex("#00FFFF")))
    magenta_ball = Sphere(Point(-0.75, -0.1, 2.25), 0.6, Material(Color.from_hex("#FF00FF")))
    
    chessboard_ball = Sphere(Point(0, 10000.5, 1.0), 10000.0, ChessMaterial(color1 = Color.from_hex("#420500"), color2 = Color.from_hex("#E6B87D"), ambient = 0.2, reflection = 0.2))
    
    light1 = Light(Point(1.5, -0.5, -10.0), Color.from_hex("#FFFFFF"))
    light2 = Light(Point(-0.5, -10.5, 0.0), Color.from_hex("#E6E6E6"))

    objects = [cyan_ball, magenta_ball, chessboard_ball]
    ...
    with open("D:/teplpp/test3.ppm","w") as img_file:
...
def main():
    ...
    red_b = Sphere(Point(0.750, -0.1, 1.0), 0.6, Material(Color.from_hex("#FF0000"), reflection=0.1))
    gre_b = Sphere(Point(-0.750, -0.1, 1.0), 0.6, Material(Color.from_hex("#00FF00"), diffuse= 0.1))

    l = Light(Point(-0.6, -10.5, 1.5), Color.from_hex("#E6E6E6"))
    ll = Light(Point(-0.4, -10.5, -1.5), Color.from_hex("#E6E6E6"))
    lll = Light(Point(-0.4, -10.5, 1.5), Color.from_hex("#E6E6E6"))
    llll = Light(Point(-0.6, -10.5, -1.5), Color.from_hex("#E6E6E6"))
    objects = [chessboard, red_b, gre_b]
    lights = [l, ll, lll, lll]
    ...
    with open("D:/teplpp/no_focus_2k.ppm","w") as img_file:
        image.write_ppm(img_file)
